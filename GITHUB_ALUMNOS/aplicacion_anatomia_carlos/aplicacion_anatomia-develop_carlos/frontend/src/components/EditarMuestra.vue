        ...
        <form class="form-muestras" @submit.prevent="searchMuestras">
          ...
          <div>
            <ul v-for="m in filteredmuestras" :key="m.codigo">
              <li  class="muestras"><router-link :to="`/informe/${m.codigo}/${m.fecha_recepcion}/${m.naturaleza}/${m.formato}/${m.organo}/${m.sede}/${m.usuario.username}/${m.calidad}/${m.interpretacion}/${m.descripcionCal}/${m.descripcionInt}`">{{ m.codigo }} - {{ m.naturaleza }} - {{ m.formato }} - {{m.sede}} - {{m.usuario}} - {{m.calidad}} - {{m.interpretacion}}</router-link></li>
            </ul> 
          </div>
